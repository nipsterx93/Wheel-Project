...
Nel repository convivono due plugin: quello nuovo (User.PluginSdkDemoRemastered/, SimRIG
Remastered, in costruzione un modulo alla volta) e quello vecchio (User.PluginSdkDemoEdit/,
congelato).

LEGGI IN QUESTO ORDINE, SENZA SALTARE:

1. AGENTS.md
   Regole operative, protocollo del lock, ciclo di un modulo del plugin nuovo, trappole del repo.
   Include la sezione "Sessioni di revisione" se il tuo compito è rivedere invece di implementare.
   Valgono per ogni agente: CLAUDE.md e GEMINI.md sono solo puntatori a questo file.

2. .ai/PROJECT_STATE.md
   Parti dal blocco LOCK (se owner non è NONE e non sei tu, NON tocchi codice), poi "Da dove
   partire" e la tabella "Avanzamento" del plugin nuovo.

3. .ai/plans/2026-09-15-remastered-spec.md
   Il design approvato del plugin nuovo: decisioni, contratti dei moduli, ordine dei passi, soglie.

4. .ai/ARCHITECTURE.md
   Mappa dei moduli del plugin nuovo e ADR. Leggi almeno ADR-004 (come si verifica un fix),
   ADR-005 (perché un campione singolo non è una misura) e ADR-007 (perché il plugin nuovo).
...
- a che passo siamo (tabella Avanzamento) e qual è il prossimo passo concreto;
...
- Ogni modulo del plugin nuovo parte da un brainstorming con me: cosa deve fare, cosa legge, cosa
  calcola, cosa mette a disposizione degli altri moduli. Solo dopo si implementa, e il modulo fa solo
  quello che dice la sua scheda.
- Ogni logica segue ADR-004: la decisione in una funzione pura, il test che la chiama davvero, e la
  verifica che il test diventi rosso neutralizzandola. I casi usano numeri veri presi dai log, non
  valori inventati.
...
Il blocco per il lavoro sulle proiezioni di fine gara del plugin vecchio (aggiornato il 2026-08-31) è stato spostato
parola per parola in `.ai/archive/PLUGIN_VECCHIO.md`.
