...
| Y-14 | Scope gomme non ricostruibile da un replay puro | `TyreManager.CurrentScope` cambia solo via `CycleTyreScope()`, chiamato dai tasti volante (`TyreManager.cs:89`) — nessun percorso lo deriva dalla telemetria. In un replay riprodotto senza un pilota che preme i tasti, resta a `None` per default, indipendentemente da cosa sia successo davvero. Effetto concreto: il ramo "solo gomme" della Fase 4 (`ObserveNaturalPitStop`) non scatta mai durante un replay, anche su una sosta con cambio gomme vero — non si può imparare `TyreChangeTime` da un replay. Il lato benzina non è affetto: `litresAdded`/`fuelingSeconds` vengono dalla variazione reale di `CurrentFuelLevel`, non dallo scope. **Non è un difetto della Fase 4** (pensata per sessione live), è un limite di cosa si può verificare da un replay. Possibile soluzione futura: stimare il cambio gomme dal tempo fermo osservato invece che dallo scope, come già fa `OpponentTracker.cs` (`opponentTiresChanged`) per gli avversari — stessa famiglia della Fase 6, non la Fase 6 stessa. |
