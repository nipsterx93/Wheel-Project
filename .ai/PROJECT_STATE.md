...
```yaml
owner:      antigravity
since:      2026-09-06T13:48:00+02:00
task:       migrazione comandi /new-session e /handoff in .agent/skills/ per Antigravity
scope:      .agent/skills/
expires:    2026-09-06T14:48:00+02:00
```
...
| Y-52 | Metadati di sessione dallo YAML (4 passi) | 🟡 **Passi 1 e 2 di 4 fatti** (Passo 1: `bd00979`; Passo 2: 2026-09-05). Nasce dall'analisi di irdashies: iRacing pubblica nel SessionInfo YAML dati che oggi rimpiazziamo con ripieghi cablati. **La premessa era gia' verificata sul campo**: `TelemetryReader.cs:90-94` legge lo YAML con quattro fallback e i log di gara lo provano — `BoP Pct: 0.500` non può venire da altro che dal parsing di `CarClassMaxFuelPct`. **Passo 1 (fatto):** `SessionMetadata` (contenitore agnostico, campi nullable) + `SessionYamlParser` (fornitore iRacing, funzione pura) + cache per sessione in `TelemetryReader` + dump `SimRigMetadata.json` su cartella configurabile. **Passo 2 (fatto):** seeding `DriverCarEstLapTime` e `CarClassEstLapTime` nei ripieghi di passo di `RaceAnalyzer.cs` (player, leader e avversari al via) e `TargetStrategyManager.cs`. Introdotto `IsLapsPredictionValid` su `RaceAnalysisResult` e pubblicato su SimHub come `SimRIG.Session.IsLapsPredictionValid` (true su gare a giri o gare a tempo con passo misurato/YAML; false fuori gara o su mero ripiego forfettario). 15 unit test in `PredictedPaceUnitTests.cs`. Risolto il "buco nero dei primi 3 giri" (a Road Atlanta GT3 dava 23 giri con ripiego a 120s; ora 36 giri al semaforo verde). **Passi 3-4 (da fare):** (3) `DriverPitTrkPct` → distanza metrica alla piazzola box; (4) densità carburante, incidenti, gomme, partenza. ⚠️ **Il vincolo del passo 2:** sul carburante "non lo so" ha uno stato silenzioso e innocuo (`IsPredictionValid` → `FuelToAdd` resta 0, la macro non parte). Sulla stima giri **non esiste**: `RaceLifeTimeLeftSec = 0` significa "gara finita adesso" a valle. Togliere il ripiego cablato richiedeva quindi un **flag di validità propagato**, ora presente come `IsLapsPredictionValid`. ⚠️ **Limite dichiarato:** la semina paga **dal semaforo verde**, non dalla griglia — prima del via `SessionTimeLeft` vale `-1` (`RaceAnalyzer.cs:1984`) e `TimeUntilLeaderCheckered` lo blocca a zero, qualunque passo si semini. ⚠️ **Lo YAML non contiene alcun consumo per giro** (soli campi carburante: `CarClassMaxFuelPct`, `DriverCarFuelMaxLtr`, `DriverCarMaxFuelPct`, `DriverCarFuelKgPerLtr`). Il freeze sul carburante **resta necessario e corretto**, e dura già un solo giro: `AverageFuelPerLap` si popola col primo giro valido. |
